...
## Installation
The requirements can be intalled in a Conda environment as follows:
...
- `num_corners_x` and `num_corners_y`: number of corners in the calibration pattern (numer of squares - 1)
...
Once the calibration is done, the following scrit can be used to visualize the calibration in a 3D plot again:
...
- `calibration_plots`: Folder containing figures to evaluate the quality of the calibration:
    - `detected_points`: figures with the combination of all the cornets extracted for each camera
