...
def main():
    ...
    if using_masks:
        iterator = zip(color_paths, masks_paths, img_names)
    else:
        iterator = zip(color_paths, color_paths, img_names)


    coolPrint('Processing images...')
    # Here we start iterating over every image
    for color_p, mask_p, file_name in tqdm(iterator, total=len(color_paths), colour='cyan'):
        ...
        if visualize_video:
            ...
            show_depth('Depth', corrected_output_float, Z_near=Z_near, Z_far=Z_far)
